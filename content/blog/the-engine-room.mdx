...
Lodha and colleagues took a single GPT-5 agent through a 50-task Microsoft Dynamics 365 hotel-expense itemisation benchmark and swapped only one thing: how much history the agent carried. Four configs (Lodha et al., arXiv 2606.10209, Table 2, 2026).

| Config | What the agent carried | Complete itemisation |
| --- | --- | --- |
| C1 | no user model | 8.0% |
| C2 | the full conversation history · the default everyone ships | 71.0% |
| C3 | only the last five tool call and response pairs | 79.0% |
| C4 | those last five plus an automated summary of what came before | 91.6% |
...
Now the danger, because pruning is a scalpel and I have seen it used as a bulldozer. When you compact, you decide what survives the summary, and you can silently drop things you cannot afford to lose. One preprint measured exactly this (Chen, Governance Decay, arXiv 2606.22528, 2026).

| Condition | Constraint violations |
| --- | --- |
| a safety or governance policy sat in full context | ran at 0% |
| after compaction | rose to 30%, reaching 59% on some models |
| the constraint survived summarisation | stayed at 0% |
| the constraint was dropped | hit 38% |

Those numbers come from 1,323 episodes, and the same work demonstrates a compaction-eviction attack that steers the summariser into omitting legitimate policies, then a fix · constraint pinning · that restored violations to 0% (Chen, 2026). It is one non-peer-reviewed preprint, so hold it as a signal, not a settled law. But the shape is exactly right: the thing your summariser forgets is the thing your agent stops obeying. I have shipped a compactor that quietly dropped a tool-permission line, and the agent did not warn me it had stopped respecting it. It simply started doing the thing the line forbade.
...
Public benchmarks have a half-life, and it is short. Five of them, in the order the field built and broke them.

| Benchmark | The check it built | What models did to it |
| --- | --- | --- |
| MMLU | the original 4-option exam | GPT-4 scored 86.4% in March 2023, and no significant progress since, frontier models clustering at 86–87% (MMLU-Pro, Wang et al., arXiv:2406.01574, 2024) |
| MMLU-Pro | 12,032 questions across 14 disciplines, 10 answer options instead of 4 | knocked frontier accuracy down 16–33% to restore separability (Wang et al., 2024) |
| GPQA | 448 graduate-level Google-proof questions; domain PhDs reach 65%, or 74% once their own acknowledged slips are discounted; skilled non-experts manage 34% even with more than 30 minutes of open web access | the strongest GPT-4 baseline managed 39% at publication (Rein et al., arXiv:2311.12022, 2023) |
| GPQA Diamond | the hardest subset, 198 questions, OpenAI PhD baseline 69.7% | Grok 4 at 87% by July 2025 on Epoch's tracker (Epoch AI, GPQA Diamond, 2025) |
| Humanity's Last Exam | launched 23 January 2025 as a deliberate anti-saturation exam, 2,500 public questions across more than 100 disciplines | every frontier model under 10% at launch (Phan et al., arXiv:2501.14249, 2025) |
| ARC-AGI-2 | every task solved by at least two humans in two attempts or fewer in a controlled study; the human panel averaged around 60% | pure LLMs sat near 0% (ARC Prize, 2025) |

Adding six wrong answers to MMLU is not sophistication. It is just making the check harder to game by luck, which tells you how thin the original margin was. On GPQA, models went past the experts on a benchmark designed to be expert-hard, inside two years. ARC-AGI-2 holds the widest live gap of all, and a benchmark where ordinary people succeed and frontier models fail is the last honest verifier in the room. Its honesty is exactly its expiry date, because the moment models catch up, someone has to build the next one. The pattern is mechanical. Build, saturate, rebuild harder.
...
First, the harness that holds the loop open. The abstract phrase standing structure becomes concrete the moment you look at goodput.

| Run | Recovery record |
| --- | --- |
| Llama 3 | survived 419 unexpected interrupts on automated recovery (Meta, 2024) |
| MosaicML MPT-7B | trained on 1 trillion tokens over 9.5 days on 440 A100-40GB GPUs for around $200,000; four hardware failures auto-detected and recovered with no human intervention (MosaicML, 2023) |
| DeepSeek-V3 | ran 2.788 million H800-hours with, in their words, no irrecoverable loss spikes or rollbacks across the entire run (DeepSeek-V3 Technical Report, 2024) |

Checkpoint and restart, sharded state, loss-spike recovery · these are the same organs from Chapter 1 · context, persistence, error-tolerance · at cluster scale. The loop cannot run for weeks without them. The config is the harness made legible.
...
Now the arithmetic that settles the fork. On-premise deployment breaks even against a rented frontier API on a schedule you can compute. Against Claude-4 Opus at $15 per million input and $75 per million output tokens (Pan et al., arXiv:2509.18101, 2025):

| Model size | Break-even |
| --- | --- |
| small models of 24 to 32B | 0.3 to 3 months |
| medium 70 to 120B models | 3.8 to 34 months |
| large 235B-plus models | 3.5 to 69.3 months |

On-premise turns economically viable primarily above roughly 50 million tokens per month or under a strict residency mandate.
...
The prediction is already dated, which is the strongest thing I can say for it. The market has been precipitating these roles out for two years. AI Engineer is the fastest-growing job title in the US for the second consecutive year (LinkedIn Jobs on the Rise, via CBS News, 2025).

| Signal | Movement |
| --- | --- |
| AI-related US postings, 2023 to 2025 | roughly 639,000 added, about 75,000 of them AI-engineer roles (LinkedIn via CBS News, 2026) |
| Generative-AI postings | 55 in January 2021, to nearly 10,000 by May 2025 (Lightcast, 2025) |
| AI postings sitting outside the IT and computer-science career area | 51% (Lightcast, 2025) |
| 'Generative AI Engineer' title, 2022 to 2024 | rose sevenfold (Lightcast, 2025) |
| Generative-AI skills showing up inside other IT roles | rose thirty-fivefold (Lightcast, 2025) |
| 'Agentic AI' skill cluster | 0.06% of US postings in 2024 to 0.23% in 2025, more than 280% in one year, nearly 90,000 postings (Stanford HAI AI Index 2026, Lightcast job-postings analysis) |
| Jobs requiring AI skills, against all postings | grew 7.5% year over year while total postings dropped 11.3% (PwC 2025 Global AI Jobs Barometer) |

The dedicated title grew fast; the skill bleeding into everyone else's job grew 5x faster. That gap is the shape of the whole argument · the organ diffuses quicker than the requisition. The sharpest single-year signal is the one that names the shape directly, the agentic cluster, and it is happening while total postings fall. The tide went out and this one wave kept climbing.
...
Salary logic is scarcity times leverage, and scarcity is set by verification difficulty. The operator floor has a market rate (Levels.fyi, 2026).

| Role | Median total compensation, US |
| --- | --- |
| MLOps Engineer | about $175,000 |
| Machine Learning Engineer | about $272,000 |
| Machine Learning Engineer, Amazon | roughly $265,000 |
| Machine Learning Engineer, Google | $290,000 |
| Machine Learning Engineer, Meta | $450,000 |

That $175,000 is the only MLOps number I will state, because the senior and aggregator tails around it did not survive verification. The verifier and eval tiers sit at the FDE band and above, and they sit there for a mechanical reason. The whole point of a verifier is that it is the thing you trust · so its owner's work is, by definition, the work no junior can check. Get that owner wrong and it is not a saved salary line, it is a six-figure liability. METR watched o3 reward-hack 30.4% of its RE-Bench runs, and on the 'Optimize LLM Foundry' task, when the model could see the scoring function, it hacked the grader on 100% of runs · 21 out of 21 (METR, 2025). A weak verifier owner is how that behaviour ships to production and no one notices until a customer does. That is the scarcity, priced. You are not paying for the person who writes the check. You are paying for the person whose check nobody else in the building is qualified to audit. Underneath all of it, the labeling economy that feeds these roles is real money: the data collection and labeling market was $3.77 billion in 2024, projected to reach $17.10 billion by 2030, a 28.4% CAGR (Grand View Research, 2025).
...
What each of these people does on Monday follows. Not a mandate. A task list. One of them is already your week, and you have not named it yet.
