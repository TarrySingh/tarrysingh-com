...
Vendor case studies on semantic caching cluster around dramatic claims.

| Source | Claim |
| --- | --- |
| One widely-cited deployment ([summary at Maxim](https://www.getmaxim.ai/articles/semantic-caching-for-llms-cut-ai-costs-and-latency-with-an-enterprise-ai-gateway/)) | Dropped a $47,000 monthly LLM bill to $12,700, a 73% cut, by pushing cache hit rates from 18% to 67% |
| AWS published research on 63,796 chatbot queries ([AWS results via Maxim](https://www.getmaxim.ai/articles/top-semantic-caching-solutions-for-ai-applications-in-2026/)) | 86% cost reduction at the optimal threshold |
| Redis | Claims up to 70% token savings on LangCache |

Take each of these with the same posture you would take to any hyperscaler benchmark: the curve is real, the magnitude is workload-specific, the upper end is the marketing number.
...
Hallucination amplification. If the underlying model hallucinated once, every semantically-similar request that follows gets served that hallucination from cache. With every repeat, the error compounds. Sampling-based ground-truth checks are the only way to detect it.
