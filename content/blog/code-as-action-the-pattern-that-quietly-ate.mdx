...
Code-as-action removes the bus. The model writes a short program; the program calls the tools; the large payloads stay inside the execution environment; only the summary comes back. The reason is simple: 
...
The 98.7 per cent is one cherry-picked scenario: a large payload moving between two tools, the single worst case for the old pattern. An independent test by AIMultiple, using a different setup, landed lower: 

code execution used 78.5 per cent fewer input tokens, 165K versus 771K.

 Still large, but it tells you the headline number moves with the workload. The savings scale with payload size and call count. For an agent that makes three small, well-defined calls in a fixed order, you'll save almost nothing and add a sandbox to your ops burden. Cloudflare's own docs say it plainly: 
...
The sandbox is the architecture. The three vendors drew the hardware boundary in three different places. Cloudflare went with V8 isolates and was refreshingly candid about the trade: 
