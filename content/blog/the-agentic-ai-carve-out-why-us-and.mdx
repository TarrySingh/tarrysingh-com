...
On April 17, 2026, the Federal Reserve issued SR 26-2, superseding SR 11-7, the fifteen-year-old model-risk guidance that governed how every major bank validates quantitative models. 
...
Agentic AI falls into the gap between three functions that each partly own it. Model risk excludes it. Third-party risk reaches the vendor but not the behavior. Cybersecurity covers the perimeter but not the decision. Failure settles where each function assumes another is holding the control. I would put it more bluntly: the institution owns the liability, but the examiner holds no checklist to defend against.

By carving out GenAI and agentic AI, SR 26-2 didn't shrink the governance problem; it expanded it. The carveout pushes responsibility onto enterprise risk frameworks that don't exist yet at most banks. If I were the chief risk officer at a mid-tier US bank, I would read that footnote as an instruction to build evidence the regulator has not yet defined, knowing that the first failure will clarify the standard retroactively.
