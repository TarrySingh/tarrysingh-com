...
The prefill phase ingests a prompt and produces the first token. It runs every position of the input through every transformer layer in parallel. That is a compute-bound matrix multiply storm, with GPU utilisation often pinned in the 90 to 95 per cent range. The decode phase generates each subsequent token one at a time, sequential by construction, dominated by memory bandwidth as the model weights and KV cache get streamed through the attention units. Decode utilisation on the same GPU sits around [20 to 40 per cent](https://www.spheron.network/blog/nvidia-dynamo-disaggregated-inference-guide/).
