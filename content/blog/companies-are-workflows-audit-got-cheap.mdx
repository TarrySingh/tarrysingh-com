...
flowchart TB
  subgraph BROKEN["Broken — handoff defined, nobody executes"]
    direction LR
    B1["Marketing emits MQL"] --> B2["Salesperson never reads inbox"]
    B2 --> B3["Lead ages out · 0% conversion"]
  end
  subgraph IGNORED["Ignored — handoff documented, never wired"]
    direction LR
    I1["Legal drafts GDPR clause"] --> I2["Sits in SharePoint · 9 months"]
    I2 --> I3["EU launch violates DPA"]
  end
  subgraph INCOMPLETE["Incomplete — handoff fires, receiving system can't act"]
    direction LR
    N1["CS ticket fires with new field"] --> N2["CRM stores it"]
    N2 --> N3["Support tool can't render"]
    N3 --> N4["Resolution time 3x baseline"]
  end
