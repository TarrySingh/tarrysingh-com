...
TPU v6e delivers 32 GB HBM capacity per chip with 1,638 GiBps HBM bandwidth and 800 GBps bidirectional inter-chip interconnect bandwidth. Google does not publish TCO comparisons against Nvidia. They publish training time per dollar and inference cost per thousand tokens, because those are the metrics a finance owner can map to revenue. Your UALink ROI case should answer the same question: how many additional training runs do you complete per quarter if the fabric is 15% faster, and what is that worth compared to the cost of being six months late?
