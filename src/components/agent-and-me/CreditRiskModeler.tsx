...
const CATEGORIES: CategoryDef[] = [
  {
    ...
    tasks: [
      { id: "pd_dev", name: "PD Model Development", desc: "Build Probability of Default models using logistic regression, survival analysis, machine learning. Includes through-the-cycle and point-in-time calibration, backtesting (Binomial test, traffic light approach per CRR Art. 185), and long-run average calibration.", aiScore: 64, complexity: 9, frequency: "Quarterly", tools: ["SAS", "Python", "R", "Moody's", "Jupyter"], subtasks: ["Data prep", "Feature selection", "Model fitting", "Discrimination testing", "Calibration", "Backtesting"] },
      { id: "lgd_dev", name: "LGD Model Development", desc: "Develop Loss Given Default models covering cure rates, workout LGD, downturn LGD adjustments (EBA/GL/2019/03), collateral haircut models, discount rate determination, and incomplete workout treatment.", aiScore: 62, complexity: 9, frequency: "Quarterly", tools: ["Python", "R", "SAS"], subtasks: ["Workout analysis", "Cure rate modeling", "Downturn adjustment", "Collateral valuation", "Discount rate determination"] },
      { id: "ead_dev", name: "EAD Model Development", desc: "Model Exposure at Default including Credit Conversion Factors (CCF) for off-balance sheet items, limit utilization forecasting, facility-level analysis, and undrawn commitment modeling.", aiScore: 65, complexity: 8, frequency: "Quarterly", tools: ["Python", "SAS"], subtasks: ["CCF estimation", "Drawdown modeling", "Limit monitoring", "Facility-level analysis"] },
      { id: "credit_scoring", name: "Credit Scorecard Development", desc: "Build application and behavioral scorecards. WoE binning, IV calculation, reject inference, score calibration to PD.", aiScore: 78, complexity: 7, frequency: "Bi-Annual", tools: ["SAS", "Python", "FICO", "OptBinning"], subtasks: ["Characteristic analysis", "WoE/IV calc", "Reject inference", "Score alignment"] },
      { id: "stress_test", name: "Stress Testing Models", desc: "Develop satellite models linking macro variables to PD/LGD/EAD. Run EBA/Fed scenarios, reverse stress tests, sensitivity analysis. Scenario design/narrative and management overlay for stress results.", aiScore: 55, complexity: 10, frequency: "Quarterly", tools: ["Python", "MATLAB", "SAS", "Bloomberg"], subtasks: ["Satellite model build", "Scenario design", "Sensitivity runs", "Result aggregation", "Management overlay"] },
      { id: "ecl_model", name: "IFRS 9 / CECL ECL Modeling", desc: "Expected Credit Loss computation: Stage allocation (SICR triggers), lifetime PD term structures, forward-looking macro overlays, probability-weighted scenarios, management adjustments, and ECL attribution analysis.", aiScore: 60, complexity: 10, frequency: "Monthly", tools: ["Python", "SAS", "Moody's ImpairmentCalc", "Oracle OFSAA"], subtasks: ["Stage allocation", "Lifetime PD curves", "Macro overlay", "Scenario weighting", "ECL attribution"] },
      ...
      { id: "concentration", name: "Concentration Risk Modeling", desc: "Measure name concentration (HHI, Granularity Adjustment), sector concentration, geographic risk clustering, contagion modeling.", aiScore: 70, complexity: 8, frequency: "Quarterly", tools: ["Python", "R", "Excel"], subtasks: ["HHI calculation", "Sector decomposition", "Add-on computation"] },
      { id: "portfolio_credit", name: "Credit Portfolio Modeling", desc: "Build multi-factor portfolio credit risk models (CreditMetrics, CreditRisk+). Monte Carlo simulation for portfolio loss distributions, asset correlation estimation, tail risk analysis.", aiScore: 52, complexity: 10, frequency: "Annual", tools: ["Python", "MATLAB", "C++"], subtasks: ["Factor model design", "Correlation calibration", "Monte Carlo engine", "Loss distribution analysis"] },
      { id: "ldp_modeling", name: "Low Default Portfolio Modeling", desc: "Specialized PD/LGD estimation for portfolios with few defaults (sovereign, bank, large corporate). Pluto-Tasche method, Bayesian approaches, expert judgment elicitation, external data mapping.", aiScore: 38, complexity: 10, frequency: "Annual", tools: ["Python", "R", "Expert panels"], subtasks: ["External data mapping", "Bayesian estimation", "Expert elicitation", "Conservative margin of conservatism"] },
    ]
  },
  {
  ...
  },
  {
    ...
    tasks: [
      { id: "model_valid", name: "Developer Testing & Validation Support", desc: "Perform first-line developer testing: replication checks, benchmark/challenger models, sensitivity analysis. Support independent 2nd-line validation by providing evidence packages, walkthroughs, and outcomes analysis.", aiScore: 45, complexity: 10, frequency: "Annual", tools: ["Python", "R", "SAS"], subtasks: ["Replication testing", "Challenger build", "Outcomes analysis", "Evidence preparation", "Use test verification"] },
      ...
      { id: "model_change", name: "Model Change Management", desc: "Formal change impact assessment for model modifications: material vs. immaterial change classification, pre/post change performance comparison, change documentation, regulatory notification where required.", aiScore: 55, complexity: 8, frequency: "Per Change", tools: ["Python", "Word", "Internal workflows"], subtasks: ["Materiality assessment", "Impact analysis", "Change documentation", "Approval routing", "Regulatory notification"] },
    ]
  },
  {
    ...
    tasks: [
      ...
      { id: "reg_report", name: "Regulatory Reporting (COREP)", desc: "Provide model outputs (RWA, EL, PD/LGD/EAD parameters) feeding COREP templates (C07/C08/C09), FINREP credit quality tables, Pillar 3 disclosures. Reconcile model outputs with finance, support EBA transparency exercise.", aiScore: 65, complexity: 8, frequency: "Quarterly", tools: ["AxiomSL", "Regnology", "Excel", "Python"], subtasks: ["Model output delivery", "RWA reconciliation", "Pillar 3 narrative", "Sign-off process"] },
      { id: "adhoc_analysis", name: "Ad-Hoc Management Analysis", desc: "Respond to one-off analytical requests from CRO, Board, or regulators. Deep-dive portfolio segments, scenario what-if analysis, peer comparison, and rapid turnaround briefs on emerging risks.", aiScore: 48, complexity: 7, frequency: "Weekly", tools: ["Python", "Excel", "Jupyter", "Tableau"], subtasks: ["Request scoping", "Rapid analysis", "Result presentation", "Documentation"] },
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: "it_integration", name: "IT Systems Integration", desc: "Interface models with core banking (T24, Finacle), origination systems, collections platforms. Shared responsibility with IT teams: modeler provides specifications, validation criteria, and UAT support.", aiScore: 55, complexity: 9, frequency: "Per Project", tools: ["APIs", "Git", "JIRA", "Confluence"], subtasks: ["Requirements specification", "Validation criteria", "UAT support", "Production cutover"] },
      { id: "mlops", name: "MLOps & Model Version Control", desc: "Git-based model versioning, experiment tracking (MLflow/Weights & Biases), model registry management, CI/CD pipelines for model deployment, reproducibility assurance across development and production environments.", aiScore: 82, complexity: 7, frequency: "Ongoing", tools: ["Git", "MLflow", "Docker", "Jenkins", "Jupyter"], subtasks: ["Version control", "Experiment tracking", "Model registry", "CI/CD pipeline", "Reproducibility audit"] },
  ...
  },
  {
    ...
    tasks: [
      ...
      { id: "peer_benchmark", name: "Peer Benchmarking & Market Intelligence", desc: "Compare internal credit risk metrics against peer banks using EBA transparency exercise data, Pillar 3 disclosures, and industry surveys. Identify competitive positioning and best practice gaps.", aiScore: 65, complexity: 6, frequency: "Semi-Annual", tools: ["Python", "Excel", "EBA data", "Bloomberg"], subtasks: ["Peer data collection", "Metric comparison", "Gap analysis", "Best practice identification", "Management briefing"] },
      { id: "reg_change_impact", name: "Regulatory Change Impact Assessment", desc: "Assess impact of upcoming regulatory changes (Basel IV output floors, CRR3, FRTB) on models, RWA, and capital. Quantify implementation effort, timeline planning, and strategic response options.", aiScore: 38, complexity: 9, frequency: "Quarterly", tools: ["Python", "Excel", "Regulatory texts"], subtasks: ["Regulatory scanning", "Impact quantification", "Implementation planning", "Capital impact", "Board communication"] },
    ]
  }
]
