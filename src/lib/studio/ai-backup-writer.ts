...
- EM-DASHES: ZERO. Not one em-dash in the piece, and never two hyphens standing in for one. This is the binding anti-slop contract's number one tell. Rewrite into two sentences, or use a comma, a colon sparingly, or parentheses sparingly. Do not swap the character and move on: restructure the sentence so it no longer wants a dash.
